...
## Supports being set after _ready() (e.g. from a parent door script).
var _exit_portal_impl:Portal
@export var exit_portal:Portal:
    get:
        return _exit_portal_impl
    set(value):
        _exit_portal_impl = value
        if _exit_portal_impl != null and is_node_ready() and not _initialized:
            _post_init()
...
# Guard against double initialization (e.g. scene-set + code-set exit_portal)
